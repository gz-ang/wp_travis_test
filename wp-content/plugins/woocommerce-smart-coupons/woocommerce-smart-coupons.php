			...
			public function smart_coupons_is_valid_for_product( $valid, $product = null, $coupon = null, $values = null ) {
				...
				if ( $discount_type == 'smart_coupon' ) {
					...
					if ( sizeof( $coupon_product_ids ) > 0 ) {
						...
						if ( in_array( $product_id, $coupon_product_ids ) || ( isset( $product_variation_id ) && in_array( $product_variation_id, $coupon_product_ids ) ) || in_array( $product->get_parent(), $coupon_product_ids ) ) {
							$valid = true;
						}
					...
					if ( sizeof( $coupon_excluded_product_ids ) > 0 ) {
						if ( in_array( $product_id, $coupon_excluded_product_ids ) || ( isset( $product_variation_id ) && in_array( $product_variation_id, $coupon_excluded_product_ids ) ) || in_array( $product->get_parent(), $coupon_excluded_product_ids ) ) {
							$valid = false;
						}
					}
					...
					if ( $is_exclude_sale_items == 'yes' ) {
						...
						if ( in_array( $product_id, $product_ids_on_sale, true ) || ( isset( $product_variation_id ) && in_array( $product_variation_id, $product_ids_on_sale, true ) ) || in_array( $product->get_parent(), $product_ids_on_sale, true ) ) {
							$valid = false;
						}
					}
				}
				...
				if ( ! empty( $post ) && isset( $post['generate_and_import'] ) ) {
					...
					for ( $i = 1; $i <= $post['no_of_coupons_to_generate']; $i++ ) {
						...
						if ( $this->is_wc_gte_30() ) {
							...
							$post['exclude_product_ids'] = ( ! empty( $post['product_ids'] ) ) ? ( ( is_array( $post['exclude_product_ids'] ) ) ? implode( ',', $post['exclude_product_ids'] ) : $post['exclude_product_ids'] ) : '';
						}
