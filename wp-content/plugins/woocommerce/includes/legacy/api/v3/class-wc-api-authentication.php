...
class WC_API_Authentication {
	...
	private function check_oauth_signature( $keys, $params ) {
		...
		$signature = base64_encode( hash_hmac( $hash_algorithm, $string_to_sign, $secret, false));

		if ( ! hash_equals( $signature, $consumer_signature ) ) {
			throw new Exception( __( 'Invalid signature - provided signature does not match. '.$signature.'- provided with the signature', 'woocommerce' ), 404 );
		}
	}
