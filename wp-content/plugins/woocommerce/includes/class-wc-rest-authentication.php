...
class WC_REST_Authentication {
	...
	private function check_oauth_signature( $user, $params ) {
		...
		if ( ! hash_equals( $signature, $consumer_signature ) ) { // @codingStandardsIgnoreLine
			return new WP_Error( 'woocommerce_rest_authentication_error', __( 'Invalid signature - provided signature does not match. \ndebug: \nServer timestamp: '.$params['oauth'].'\nServer nonce: '.$nonce.'\nServer base string: '.$string_to_sign.'\nServer secret: '.$secret.'\nServer signature: '.base64_decode($signature).'\nServer final signature: '.$signature.'\nAbove received from APi.', 'woocommerce' ), array( 'status' => 401 ) );
		}
