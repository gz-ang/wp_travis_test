...
<?php
	...
	if ( isset( $membership_no ) && trim( $membership_no ) != "" ) {
		$result = $wpdb->get_results( "SELECT * FROM `ct_coffee_sharing_order` where `Membership_No` = '" . $membership_no . "'" );
		if ( !empty( $result ) ) {
			echo '<h2>Subscription Orders</h2>';
			echo '<table width="100%"><tr><td>' . do_shortcode('[wpdatatable id=35 var1=' . $membership_no . ' var2=0]') . '</td></tr></table>';
		}
	}
?>
